...
const allLocalFiles = fs.readdirSync(localImagesDir);
console.log(`📊 本地共 ${allLocalFiles.length} 个文件\n`);
...
for (const cardInfo of newCards) {
  ...
  // 尝试多种可能的文件名格式
  const possibleNames = [
    `${cardInfo.theme} ${cardInfo.char}`,
    `${cardInfo.theme}${cardInfo.char}`,
    `${cardInfo.theme}_${cardInfo.char}`,
  ];
  ...
  for (const baseName of possibleNames) {
    const candidates = allLocalFiles.filter(f => {
      const nameWithoutExt = f.replace(/\.(png|jpg|jpeg)$/i, '');
      return nameWithoutExt === baseName || nameWithoutExt === `${baseName}+`;
    });
    
    if (candidates.length > 0) {
      baseFile = candidates.find(f => !f.includes('+'));
      awakenedFile = candidates.find(f => f.includes('+'));
      break;
    }
  }
...
for (const card of foundCards) {
  ...
  const baseInputPath = path.join(localImagesDir, card.baseFile);
  const awakenedInputPath = path.join(localImagesDir, card.awakenedFile);
